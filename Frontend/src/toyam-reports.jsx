...
function TrendChart({ trends = {} }) {
  ...
  // Use pH timestamps as the common X-axis
...
function WaterQualityOverview({trends}) {
...
function ContaminantSummary({contaminants = []}) {
  return (
    <Card style={{ padding: 22, flex: "1 1 380px", overflow: "hidden" }}>
      ...
      <div style={{ overflowX: "auto" }}>
        <table style={{ width: "100%", borderCollapse: "collapse", minWidth: 460 }}>
          ...
          <tbody>
            {contaminants.map((c) => (
    <tr key={c.parameter} style={{ borderBottom: "1px solid #F5F6F8" }}>
    <td style={{ padding: "11px 8px", fontSize: 13, color: "#344054", fontWeight: 600 }}>
      {c.parameter}
    </td>

    <td style={{ padding: "11px 8px", fontSize: 13, color: "#344054" }}>
      {c.value}
    </td>
...
function TemporalHeatmap({ heatmap = [] }) {
  return (
    <Card style={{ padding: 22, flex: "2 1 460px" }}>
      <SectionTitle>
      ...
      >
        {heatmap.length === 0 ? (
          ...
          heatmap.map((item) => {
            const total =
              item.safe + item.warning + item.critical;

            const safePercentage =
              total > 0 ? (item.safe / total) * 100 : 0;

            const warningPercentage =
              total > 0 ? (item.warning / total) * 100 : 0;

            const criticalPercentage =
              total > 0 ? (item.critical / total) * 100 : 0;
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 4 }}>
        {reports.map((r) => (
          ...
          >
            ...
            {/* Download button */}
            <button
              onClick={() => console.log(onDownload(r.id))}
              style={{
                border: "1px solid #E7EBF0",
                background: "#fff",
                borderRadius: 7,
...
export default function ToyamReports() {
  ...
  try {
    const response = await fetch(
      "http://127.0.0.1:8000/api/reports/contaminants"
    );

    const result = await response.json();

    console.log("Contaminants:", result);

    setContaminants(result.data);
  } catch (error) {
  ...
  try {
    const response = await fetch(
      "http://127.0.0.1:8000/api/reports/trends"
    );
...
const fetchHeatmap = async () => {
  try {
    const response = await fetch(
      "http://127.0.0.1:8000/api/reports/heatmap"
    );
...
const fetchRecentReports = async () => {
  try {
    ...
    console.log("Recent reports:", result);
...
const downloadReport = async (filters) => {
  try {
    ...
    const response = await fetch(
      `http://127.0.0.1:8000/api/reports/download?${params.toString()}`
    );
...
const KPIS = [
  { label: "Overall Water Safety Score", value: summary?.overall_water_safety_score ?? "--", unit: "/100", sub: "Safe", subTone: "ok", delta: "3 points from previous period", up: true, icon: icons.shield, bg: "#EAF7EE", fg: "#22C55E" },
  { label: "Tests Conducted", value: summary?.tests_conducted ?? "--", sub: "Across all parameters", delta: "12% from previous period", up: true, icon: icons.flaskFill, bg: "#EFF6FF", fg: "#2563EB" },
  { label: "Alerts Triggered", value: summary?.alerts_triggered ?? "--", sub: "1 Warning, 1 Info", delta: "33% from previous period", up: false, icon: icons.warning, bg: "#FEF4E6", fg: "#E08A2E" },
  { label: "Data Uptime", value: summary?.data_uptime ?? "--", unit: "%", sub: "Excellent", subTone: "ok", delta: "0.4% from previous period", up: true, icon: icons.drop, bg: "#F3EEFF", fg: "#7C4DFF" },
];

// const contaminants = contaminants.map((item) => ({
//   name: item.parameter,
//   avg: item.value.toString(),
//   max: "--",
//   ref: item.safe_limit.toString(),
//   status: item.status === "Safe" ? "Low" : item.status,
//   tone: item.status === "Safe" ? "ok" : "warn",
// }));
  ...
  >
    ...
    Schedule Report
          ...
          <div style={{ marginBottom: 16 }}>
            <ReportFilters onGenerate={(filters) => {
    console.log("Selected report filters:", filters);

    // Your existing generate/download function
    downloadReport();
  }}/>
          </div>
          ...
          <div className="mid-row">
            <WaterQualityOverview trends={trends}/>
             <ContaminantSummary contaminants={contaminants} />
          </div>

          <div className="mid-row">
            <TemporalHeatmap heatmap={heatmap}/>
            <RecentReports reports={recentReports} onDownload={downloadReport}/>
          </div>
