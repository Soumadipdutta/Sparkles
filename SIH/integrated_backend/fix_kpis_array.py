...
for file_path in target_files:
    if os.path.exists(file_path):
        ...
        print(f"Fixed kpis in {file_path}")
...
if os.path.exists(key_path):
    ...
    if isinstance(live_kpis, list):
        ...
        for k, v in live_kpis.items():
            if isinstance(v, dict) and v.get("key") == "compliance":
                db.reference(f"statewide_overview/kpis/{k}/value").set("94.2")

    print("Pushed updated kpis array to Firebase RTDB!")
