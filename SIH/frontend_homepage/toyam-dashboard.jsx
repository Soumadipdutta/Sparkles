...
// function TopBar({ setOpen, dashboard }) {
//   return (
//     <div style={{
...
//           Village: {dashboard.info.village}
//         </div>
//       </div>
...
//         }}>
//           Last updated:{" "}
//           {new Date(dashboard.system.last_updated).toLocaleTimeString([], {
//             hour: "numeric",
//             minute: "2-digit",
//           })}
...
//           {dashboard.system.online ? "ONLINE" : "OFFLINE"}
//         </div>
//       </div>
//     </div>
//   );
...
// /* ---------- App ---------- */
// export default function ToyamDashboard({ dashboard }) {
...
//     {
//       label: "pH",
//       value: dashboard.water_quality.ph,
//       unit: "",
//       icon: icons.beaker,
//       sub: "Range: 6.5 - 8.5",
//       color: "#2F6FED",
...
//     {
//       label: "Turbidity",
//       value: dashboard.water_quality.turbidity,
//       unit: "NTU",
//       icon: icons.drop,
//       sub: "Safe: < 5 NTU",
//       color: "#2F6FED",
...
//     {
//       label: "TDS",
//       value: dashboard.water_quality.tds,
//       unit: "ppm",
//       icon: icons.drop,
//       sub: "Ideal: < 600 ppm",
//       color: "#2F6FED",
...
//     {
//       label: "Temperature",
//       value: dashboard.water_quality.temperature,
//       unit: "°C",
//       icon: icons.thermo,
//       sub: "Range: 25 - 30 °C",
//       color: "#2F6FED",
...
//         <main className="main">
//           <TopBar setOpen={setOpen} dashboard={dashboard} />

//           <div className="top-cards-row" style={{ marginBottom: 16 }}>
//             <ScoreCard score={dashboard.water_safety.score} />
//             <QuickStatus />
//           </div>
...
function formatStatus(status) {
  ...
  if (value === "within_reference") return "Within reference value";
  if (value === "maintenance_soon") return "Maintenance Soon";
...
function TrendChart({ points }) {
  ...
  // Left axis: pH / turbidity / temperature
  const scaleLeft = (v) =>
    padT + innerH - (v / 40) * innerH;

  // Right axis: TDS
  const scaleRight = (v) =>
    padT + innerH - (v / 800) * innerH;
...
// helper function
const isSafe = String(status || "").toLowerCase() === "safe";

const statusLabel = isSafe ? "SAFE TO DRINK" : "UNSAFE";
const statusColor = isSafe ? "#1E8E4E" : "#C0392B";
const statusIconColor = isSafe ? "#22C55E" : "#EF4444";

function ScoreCard({ score, changeFromYesterday, status }) {
  const circumference = 2 * Math.PI * 54;
  const offset = circumference * (1 - score / 100);
  const isSafe = String(status || "").toLowerCase() === "safe";
    ...
    <Card style={{ padding: 24, flex: "2 1 420px" }}>
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 28, flexWrap: "wrap", justifyContent: "center" }}>
        <div style={{ position: "relative", width: 140, height: 140, flexShrink: 0 }}>
          <svg width="140" height="140" viewBox="0 0 140 140">
            <circle cx="70" cy="70" r="54" fill="none" stroke="#EEF1F5" strokeWidth="12" />
            <circle cx="70" cy="70" r="54" fill="none" stroke={isSafe ? "#22C55E" : "#EF4444"} strokeWidth="12" strokeDasharray={circumference} strokeDashoffset={offset} strokeLinecap="round" transform="rotate(-90 70 70)" />
          </svg>
          <div style={{ position: "absolute", inset: 0, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center" }}>
            <span style={{ fontSize: 34, fontWeight: 800, color: "#101828", lineHeight: 1 }}>{score}</span>
            <span style={{ fontSize: 12, color: "#98A2B3", fontWeight: 600 }}>/100</span>
        ...
        <div style={{ flex: "1 1 220px", minWidth: 220 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 8 }}>
            <Icon path={icons.shield} color={statusIconColor} size={20} />
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 6, fontSize: 13, fontWeight: 600, color: "#1E8E4E" }}>
            <Icon path={icons.arrowUp} size={14} color="#1E8E4E" />
            {changeFromYesterday >= 0 ? "+" : ""}{changeFromYesterday} points from yesterday
          </div>
        </div>
      </div>
    </Card>
...
function QuickStatus({ dashboard }) {
  return (
    <Card style={{ padding: 24, flex: "1 1 260px" }}>
      <div style={{ fontSize: 12, fontWeight: 700, letterSpacing: "0.06em", color: "#98A2B3", marginBottom: 16 }}>
        QUICK STATUS
      ...
      <div style={{ background: "#EFF6FF", borderRadius: 14, padding: 18, display: "flex", gap: 12 }}>
        <div style={{ width: 34, height: 34, borderRadius: "50%", background: "#DCEBFF", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
          <Icon path={icons.check} size={16} color="#2563EB" />
        </div>
        <div>
          <p style={{ margin: 0, fontSize: 13.5, fontWeight: 600, color: "#1D3A6B", lineHeight: 1.5 }}>
            {dashboard.system.quick_status}
          </p>
          <p style={{ margin: "8px 0 0", fontSize: 13.5, color: "#3B5A8A", lineHeight: 1.5 }}>
            All systems are functioning normally.
          </p>
...
function ArsenicCard({ data }) {
  const isSafe = data.status === "within_reference" || data.status === "low";
  ...
  return (
    <Card style={{ padding: 20, flex: "1 1 300px" }}>
      ...
      <div style={{ marginBottom: 16 }}>
        <Pill tone={isSafe ? "ok" : "bad"}>
          {isSafe ? "Within reference value" : "Unsafe"}
        </Pill>
      </div>
      ...
      <div style={{
        ...
        <div style={{
          position: "absolute",
          left: `${Math.min((data.value / 0.02) * 100, 100)}%`,
          top: -3,
          width: 12,
          height: 12,
          borderRadius: "50%",
          ...
          boxShadow: `0 0 0 1px ${isSafe ? "#22C55E" : "#EF4444"}`
        }} />
      </div>
      ...
      <div style={{
        ...
        Reference value: {data.reference_limit} {data.unit}
      </div>
    </Card>
  );
}

function HeavyMetalCard({ data }) {
  const isSafe = data.status === "low";
...
function MetalsTable({ contaminants }) {
  const metals = [
    {
      name: "Lead (Pb)",
      data: contaminants.lead,
    },
    {
      name: "Iron (Fe)",
      data: contaminants.iron,
    },
    {
      name: "Manganese (Mn)",
      data: contaminants.manganese,
    },
    {
      name: "Chromium (Cr)",
      data: contaminants.chromium,
    },
  ];
      ...
      <div style={{ overflowX: "auto" }}>
        <table style={{
          ...
          <tbody>
            {metals.map((metal) => {
              const isSafe =
                metal.data.status === "low" ||
                metal.data.status === "within_reference";

              const isMonitoring =
                metal.data.status === "monitor";
              ...
              return (
                ...
                >
                  ...
                  <td style={{
                    ...
                    {metal.data.value} {metal.data.unit}
                  </td>
                  ...
                  <td style={{
                    ...
                    >
                      <polyline
                        points="0,12 8,9 16,13 24,7 32,10 40,6 46,9"
                        fill="none"
                        stroke="#98A2B3"
                        strokeWidth="1.6"
                      />
    ...
    <Card style={{ padding: 24 }}>
            ...
            <React.Fragment key={stage.label}>
              <div style={{
                ...
                <div style={{
                  ...
                  height: 56,
                  borderRadius: "50%",
                  background: isOperational ? "#EFF6FF" : "#FEF2F2",
                  ...
                  <Icon
                    path={stage.icon}
                    size={22}
                    color={isOperational ? "#2563EB" : "#EF4444"}
                  />
                </div>
                ...
                <div style={{
                  ...
                  <Icon
                    path={isOperational ? icons.check : icons.x}
                    size={12}
                    color={isOperational ? "#22C55E" : "#EF4444"}
                  />
                </div>
              </div>
      ...
      <div style={{
        ...
        {stages.every(
          ...
          ? "All treatment stages are operational"
          : "One or more treatment stages require attention"}
      </div>
    </Card>
  );
}

function TrendCard({ dashboard }) {
  const [range, setRange] = useState("24H");
    ...
    <Card style={{
      ...
      <div style={{
        ...
        <span>
          ...
          {range === "24H"
            ? "Water quality has remained stable over the last 24 hours."
            : range === "7D"
              ? "7-day trend data is currently unavailable."
              : "30-day trend data is currently unavailable."
          }
        </span>
      </div>
...
function AlertsCard({ alerts }) {
  const events = alerts?.recent || [];

  const hasActiveAlerts = events.some(
    (event) =>
      event.status === "active" ||
    ...
    <Card style={{ padding: 22, flex: "1 1 300px" }}>
      ...
      <div style={{
        ...
        <div style={{
          ...
          height: 34,
          borderRadius: "50%",
          background: hasActiveAlerts ? "#FEF2F2" : "#EAF7EE",
          display: "flex",
          alignItems: "center",
          justifyContent: "center",
          flexShrink: 0
          ...
          <Icon
            path={hasActiveAlerts ? icons.x : icons.check}
            size={16}
        ...
        <div>
          <div style={{
            fontSize: 14,
            fontWeight: 700,
            color: hasActiveAlerts ? "#C0392B" : "#1E8E4E"
          }}>
            {hasActiveAlerts
              ? "Active alerts"
              : "No active alerts"}
          ...
          <div style={{
            ...
            {hasActiveAlerts
              ? "One or more monitored conditions require attention."
...
function FilterHealth({ filters }) {
  const filterData = [
    {
      name: "Sediment Filter",
      data: filters.sediment,
      icon: icons.cylinder,
    },
    {
      name: "Carbon Filter",
    ...
    <Card style={{ padding: 22, flex: "1 1 320px" }}>
      ...
      <div style={{
        ...
        {filterData.map((filter) => {
          const isHealthy = filter.data.status === "healthy";
          const isMaintenance =
            filter.data.status === "maintenance_soon";
  ...
  return (
    <Card style={{ padding: 22, flex: "1 1 420px" }}>
      ...
      <div className="risk-grid">
        <div style={{ background: "#F9FAFB", borderRadius: 12, padding: 16 }}>
          <div style={{ fontSize: 12, color: "#98A2B3", marginBottom: 8 }}>Current Assessment</div>
          <div style={{ marginBottom: 10 }}><Pill tone={getTone(risk.risk_level)}>{String(risk.risk_level).toUpperCase()} CONTAMINATION RISK</Pill></div>
          <p style={{ fontSize: 12.5, color: "#667085", lineHeight: 1.5, margin: 0 }}>{risk.summary}</p>
          <p style={{ fontSize: 11.5, color: "#98A2B3", margin: "10px 0 0", fontStyle: "italic" }}>This is an environmental risk indicator, not a medical diagnosis.</p>
        </div>
        <div>
          <div style={{ fontSize: 12, color: "#98A2B3", marginBottom: 10 }}>Key Findings</div>
          <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
            {(risk.key_findings || []).map((f) => <div key={f.name} style={{ display: "flex", alignItems: "center", justifyContent: "space-between", fontSize: 13 }}>
              <span style={{ display: "flex", alignItems: "center", gap: 7, color: "#344054" }}><span style={{ width: 6, height: 6, borderRadius: "50%", background: getTone(f.status) === "warn" ? "#F0A03D" : "#22C55E" }} />{f.name}</span>
              <span style={{ color: "#667085", fontSize: 12.5 }}>{formatStatus(f.status)}</span>
            </div>)}
          </div>
          <a href="#" style={{ display: "inline-flex", alignItems: "center", gap: 4, fontSize: 13, color: "#2563EB", fontWeight: 600, textDecoration: "none", marginTop: 14 }}>View detailed risk analysis <Icon path={icons.arrowRight} size={13} /></a>
        </div>
      </div>
    </Card>
  );
    ...
    <div style={{ fontFamily: "'Inter', ui-sans-serif, system-ui, -apple-system, sans-serif", background: "#F6F7F9", minHeight: "100vh" }}>
      ...
      <div className="layout">
        ...
        <main className="main">
          ...
          <div className="top-cards-row" style={{ marginBottom: 16 }}>
            <ScoreCard
              score={dashboard.water_safety.score}
              changeFromYesterday={dashboard.water_safety.change_from_yesterday}
              status={dashboard.water_safety.status}
            />
            <QuickStatus dashboard={dashboard} />
          </div>
          ...
          <Card style={{ padding: 22, marginBottom: 16 }}>
            <SectionTitle>Contaminant Monitoring</SectionTitle>
            <div className="contam-row">
              <ArsenicCard data={dashboard.contaminants.arsenic} />
              <HeavyMetalCard data={dashboard.contaminants.heavy_metal_index} />
              <MetalsTable contaminants={dashboard.contaminants} />
            </div>
          </Card>
          ...
          <div style={{ marginBottom: 16 }}>
            <Pipeline
              stages={[
                { label: "SOURCE", icon: icons.beaker, status: dashboard.purification.stages.source.status },
                { label: "SEDIMENT", icon: icons.filter, status: dashboard.purification.stages.sediment.status },
                { label: "CARBON", icon: icons.cylinder, status: dashboard.purification.stages.carbon.status },
                { label: "UF/RO", icon: icons.tubes, status: dashboard.purification.stages.uf_ro.status },
                { label: "UV", icon: icons.thermo, status: dashboard.purification.stages.uv.status },
                { label: "CLEAN WATER", icon: icons.glass, status: dashboard.purification.stages.clean_water.status },
              ]}
            />
          </div>
          ...
          <div className="mid-row" style={{ marginBottom: 16 }}>
            <FilterHealth filters={dashboard.filters} />
            <RiskCard risk={dashboard.risk_awareness} />
          </div>
